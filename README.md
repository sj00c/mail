...
브라우저에서 **http://localhost:8787** 접속 → **"Gmail 연결하기"** → 본인 계정 선택 → 허용.
...
### 6. 평소 사용 & 코드 수정용 개발 모드 (선택)

평소엔 위 **프로덕션 모드**(`bun run build && bun run start`, **http://localhost:8787**)를 그대로 쓰면 된다 — 프로세스 하나가 SPA까지 서빙한다(gzip·장기 캐시). **북마크는 `8787`로.**

코드를 직접 고칠 때만 핫리로드용 개발 모드를 쓴다:

| 모드 | 명령 | 주소 | 용도 |
|---|---|---|---|
| **프로덕션** | `bun run build && bun run start` | `localhost:8787` | **일상 사용** — 단일 프로세스가 SPA까지 서빙 (gzip, 장기 캐시) |
| 개발 | `bun run dev` | `localhost:5173` | 코드 수정·핫리로드 (Vite + API 서버 2개 프로세스) |

> `5173`은 `bun run dev`를 켜둔 동안만 뜨는 개발용 포트다. 자동 시작을 등록해도 `5173`은 열리지 않는다 — 북마크는 `8787`로.
