...
3. **Gmail API + Google Calendar API** 둘 다 Enable
4. **OAuth 동의 화면**: External + **Test users에 본인 Gmail 추가**
5. **OAuth client ID(Web)** 생성 + redirect URI `http://localhost:8787/auth/callback`
...
- [Bun](https://bun.sh) 이 필요하다 (`curl -fsSL https://bun.sh/install | bash`).
...
콘솔의 "API 키 만들기"는 무시. 우리가 만드는 건 **OAuth client ID(Web application)** 다.

흐름: `프로젝트 생성 → API 2개 Enable → OAuth 동의 화면 → OAuth client ID(Web) → .env → 실행 → 로그인`
...
## 2. 프로젝트 생성 + API 활성화 (Gmail + Calendar)

1. https://console.cloud.google.com → 상단 프로젝트 선택 → **New Project** (이름 아무거나)
2. APIs & Services → **Library** 에서 각각 검색 후 **Enable**:
   - **Gmail API**
   - **Google Calendar API**  ← 캘린더 탭용. **"CalDAV API" 아님!**
- ★ 둘 다 먼저 안 켜면 클라이언트 만들어도 호출이 막힌다.
  - 캘린더에서 `403 ... Calendar API has not been used in project` 뜨면 이 단계를 빼먹은 것.

---

## 3. OAuth 동의 화면 (OAuth consent screen / 새 UI는 "Audience")

- APIs & Services → **OAuth consent screen**
- User Type: **External** → Create
- 앱 이름 / 지원 이메일 등 필수값만 입력
- **Test users → + ADD USERS** 에 **로그인할 본인 Gmail 주소** 추가
  - ★ 이거 안 하면 로그인 시 `403 access_denied` (앱이 Testing 상태라 등록된 테스터만 허용)
  - ★ 주소 오타 나면 똑같이 막힘. 정확히 입력.
- 앱은 **"Testing" 상태로 그냥 둔다** (개인용이라 게시/검수 불필요)
  - 참고: Testing 상태에선 refresh token이 7일마다 만료될 수 있음 → 만료되면 다시 로그인하면 됨.

---

## 4. OAuth client ID 생성 (+ redirect URI)

- APIs & Services → **Credentials → Create Credentials → OAuth client ID**
- Application type: **Web application** ← ★ 반드시 이거
  - ★ Desktop / TV 등으로 만들면 **Authorized redirect URIs 입력칸이 아예 안 나온다**
    (이름만 수정 가능한 화면이 뜸). 타입은 생성 후 변경 불가 → 잘못 만들었으면 삭제하고 새로 생성.
- **Authorized redirect URIs → + ADD URI** 에 정확히:
  ...
  - ★ 포트를 바꿀 거면(아래 6번 `PORT`) 여기 URI의 포트도 **똑같이** 바꿔야 한다.
- **Create** → 뜨는 팝업(또는 다운로드 JSON)에서 **Client ID** 와 **Client secret** 복사
...
- ★ `OAUTH_REDIRECT` 포트 = `PORT` = 콘솔 redirect URI 포트, **세 군데가 글자 하나까지 동일**해야 함.
...
| `Calendar API has not been used` (403) | Calendar API 미활성화 | Library에서 Google Calendar API Enable |
| 인증은 되는데 토큰 교환 실패 | .env 시크릿에 따옴표/공백 | 따옴표 제거, raw 값만 |
| OAuth 후 빈 페이지/8787로 튕김 | dev인데 8787로 떨어짐 | 정상. dev는 콜백 후 5173으로 자동 복귀하게 돼 있음 (안 되면 `bun run dev` 재시작) |
...
`.env` 의 `PORT` 와 `OAUTH_REDIRECT`, 그리고 **콘솔의 Authorized redirect URIs** 세 군데를
