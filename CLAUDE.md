...
7. Do not change the fixed port or expose the server to the LAN.
8. Run the installer only after `.env` exists and the user confirms they saved both credential values.
9. Verify the build and `http://127.0.0.1:8787/auth/status`. Report the exact failing step when installation does not complete.
